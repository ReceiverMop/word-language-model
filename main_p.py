...
def build_init_weights_matrix():
    ...
    mat=ss.load_npz('svd_reduced_mat_200_sym_ant_pats_beta1.npz')
    ...
    for w in corpus.dictionary.word2idx:
        ...
        if w in cws_clean:
            row_old=cws_clean[w]
            new_mat[row_new]=mat[row_old].toarray()
            ...
            missing.append(w)
            new_mat[row_new]=numpy.random.uniform(low=-10, high=10, size=(args.emsize,))
...
def compute_spearman(pair_list_human):
    ...
    print "The spearman corr is: ",  round(spearman_rho[0], 5)
    
    return round(spearman_rho[0], 5)
